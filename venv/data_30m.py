...
def STOCH(symbol):
    ...
    filename = "data/datadump/indicator/30min/" + indicator + symbol + now + ".json"
...
def getMACD(symbol):
    try:
        ...
        now = now.strftime("%d%m%Y%H:%M")
        filename = "data/datadump/indicator/30min/" + indicator + symbol + now + ".json"
        url = "https://www.alphavantage.co/query?function=MACD&interval=30min&series_type=close&apikey=6OKA3205BSYK5H0G&symbol=" + symbol
        payload = {}
        headers = {}
...
def getSTOCH(symbol):
    try:
        ...
        now = now.strftime("%d%m%Y%H:%M")
        filename = "data/datadump/indicator/30min/" + indicator + symbol + now + ".json"
        url = "https://www.alphavantage.co/query?function=STOCH&interval=30min&series_type=close&apikey=6OKA3205BSYK5H0G&symbol=" + symbol
        payload = {}
        headers = {}
        response = requests.request("GET", url, headers=headers, data=payload, proxies=proxyList(), timeout=5)
        with open(filename, 'w') as outfile:
...
def getRSI(symbol):
    try:
        ...
        now = now.strftime("%d%m%Y%H:%M")
        filename = "data/datadump/indicator/30min/" + indicator + symbol + now + ".json"
        url = "https://www.alphavantage.co/query?function=RSI&interval=30min&time_period=14&series_type=close&apikey=6OKA3205BSYK5H0G&symbol=" + symbol
        payload = {}
        headers = {}
        response = requests.request("GET", url, headers=headers, data=payload, proxies=proxyList(), timeout=5)
        with open(filename, 'w') as outfile:
